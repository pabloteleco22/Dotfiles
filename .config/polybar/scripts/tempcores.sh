...
do
    ...
    do
        if (( "${tempCore[$iCore]}" < "${temperaturesValues[$iTemp]}"  )); then
            tmpEcho="${tempCore[$iCore]}$degree%{O3}%{u-}"
...
do
    if (( "$sum" < "${temperaturesValues[$iTemp]}" )); then
        ...
        tmpEcho="%{u${temperaturesColors[$iTemp]}}%{+u}%{F${temperaturesColors[$iTemp]}}${temperaturesIcons[$iTemp]}%{F-}"
        ## This line will NOT color the icon
        #tmpEcho="${temperaturesIcons[$iTemp]}"
        finalEcho="$tmpEcho %{O2} $finalEcho"
        break
    fi
done
